...
class ScoringSystemTests {
	...
	void setUp() {
		...
		system = new ScoringSystem(name: "Test", scoringRules: new HashSet<ScoringRule>()).save(flush: true)
		system.scoringRules.add(passingYardsRule)
		system.scoringRules.add(passingTouchdownRule)
	...
	void testScoringRulesNullable() {
		system = new ScoringSystem(name: "Test")

		assertTrue system.validate()

		assert !system.hasErrors()
	}

	void testCalculateScore_PassingYards() {
		def stat = new Stat(statKey: FantasyConstants.STAT_PASSING_YARDS, statValue: 10)
		List stats = [stat]
	...
	void testCalculateScore_PassingTouchdowns() {
		Stat stat = new Stat(statKey: FantasyConstants.STAT_PASSING_TOUCHDOWNS, statValue: 3)
		List stats = [stat]
	...
	void testCalculateScore_PassingYardsAndPassingTouchdowns() {
		def passingTDs = new Stat(statKey: FantasyConstants.STAT_PASSING_TOUCHDOWNS, statValue: 3)
		def passingYards = new Stat(statKey: FantasyConstants.STAT_PASSING_YARDS, statValue: 10)

		List stats = [passingTDs, passingYards]

		double value = system.calculateScore(stats)
		assertTrue("Expected 12.4, got " + value, value == 12.4)

		passingTDs.statValue = 1
		value = system.calculateScore(stats)
		assertTrue("Expected 4.4, got " + value, value == 4.4)
	}
